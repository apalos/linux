...
static int cpsw_sp_port_vlan_add(struct cpsw_priv *priv,
				 u16 vid, bool untagged, bool pvid)
{
	...
	if (priv->sw_port) {
		...
		if (cpsw->version == CPSW_VERSION_1)
			reg = CPSW1_PORT_VLAN;
		/* no barrier ?? */
		slave_write(cpsw->slaves + priv->sw_port - 1, port_vlan, reg);
	} else {
		port_vlan_reg = &cpsw->host_port_regs->port_vlan;
		writel(port_vlan, port_vlan_reg);
	}
	dev_info(priv->dev, "VID: %u dev: %s port: %u\n", vid,
		 priv->ndev->name, priv->sw_port);

	return ret;
}

static int cpsw_sp_port_vlan_del(struct cpsw_priv *priv, u16 vid)
{
	int ret = 0;
	struct cpsw_common *cpsw = priv->cpsw;
...
static int cpsw_sp_port_vlans_add(struct cpsw_priv *priv,
				  ...
				  struct switchdev_trans *trans)
{
	...
	for (vid = vlan->vid_begin; vid <= vlan->vid_end; vid++) {
		...
		err = cpsw_sp_port_vlan_add(priv, vid, untagged, pvid);
		if (err)
			return err;
	}
...
static int cpsw_sp_port_vlans_del(struct cpsw_priv *priv,
				  const struct switchdev_obj_port_vlan *vlan)
{
	u16 vid;
	...
	for (vid = vlan->vid_begin; vid <= vlan->vid_end; vid++) {
		...
		err = cpsw_sp_port_vlan_del(priv, vid);
		if (err)
			return err;
	}
			     ...
			     struct switchdev_trans *trans)
{
	...
	case SWITCHDEV_OBJ_ID_PORT_VLAN:
		cpsw_sp_port_vlans_add(priv, vlan, trans);
		break;
...

{
	...
	switch (obj->id) {
	case SWITCHDEV_OBJ_ID_PORT_VLAN:
		cpsw_sp_port_vlans_del(priv, vlan);
		break;
	case SWITCHDEV_OBJ_ID_PORT_MDB:
		break;
	default:
...
void cpsw_sp_port_switchdev_init(struct net_device *ndev)
{
